...
jest.mock("../config/database", () => ({
  queryWrite: jest.fn(async (text: string, params?: any[]) => {
    if (text.includes("INSERT INTO transactions")) {
      const [id, reference_number, type, amount, phone_number, provider, stellar_address, status] = params!;
      mockTransactions.push({ id, reference_number, type, amount, phone_number, provider, stellar_address, status });
    } else if (text.includes("INSERT INTO disputes")) {
      const [id, transaction_id, reason, status, priority] = params!;
      mockDisputes.push({ id, transaction_id, reason, status, priority, created_at: new Date(), sla_due_date: null });
    } else if (text.includes("UPDATE disputes SET created_at = NOW() - INTERVAL '5 hours'")) {
      const [id] = params!;
      ...
      mockDisputeNotes.push({ dispute_id, author, note, created_at: new Date() });
    }
    return { rows: [], rowCount: 1 };
  }),
  queryRead: jest.fn(async (text: string, params?: any[]) => {
    if (text.includes("SELECT id, priority, created_at AS \"createdAt\" FROM disputes WHERE sla_due_date IS NULL")) {
      const res = mockDisputes
        .filter(d => d.sla_due_date === null)
        .map(d => ({ id: d.id, priority: d.priority, createdAt: d.created_at }));
      return { rows: res };
    } else if (text.includes("status IN ('open', 'investigating')") && text.includes("sla_due_date < NOW()")) {
      const res = mockDisputes
        .filter(d => ["open", "investigating"].includes(d.status) && d.sla_due_date !== null && new Date(d.sla_due_date) < new Date())
        .map(d => ({ id: d.id, transactionId: d.transaction_id }));
      return { rows: res };
    } else if (text.includes("SELECT provider, phone_number AS \"phoneNumber\"")) {
      const [id] = params!;
      const tx = mockTransactions.find(t => t.id === id);
      return {
        rows: tx ? [{ provider: tx.provider, phoneNumber: tx.phone_number, amount: String(tx.amount) }] : [],
      };
    } else if (text.includes("SELECT status, resolution, sla_due_date FROM disputes")) {
      const [id] = params!;
      const d = mockDisputes.find(x => x.id === id);
      return {
        rows: d ? [{ status: d.status, resolution: d.resolution, sla_due_date: d.sla_due_date }] : [],
      };
...
jest.mock("../services/mobilemoney/mobileMoneyService", () => ({
  MobileMoneyService: jest.fn().mockImplementation(() => ({
    sendPayout: jest.fn().mockResolvedValue({ success: true, data: { transactionId: "mock-payout-tx-id" } }),
  })),
}));
...
describe("Dispute Model Integration and SLA Resolution Engine", () => {
  test("should set SLA timelocks and auto-resolve overdue disputes with payout", async () => {
    ...
    await queryWrite(
      ...
      [txId, "TXN-REF-SLA", "deposit", 200, "+237600000000", "mock", "GA", "completed"]
    );
